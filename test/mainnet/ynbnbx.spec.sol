...
contract YnBNBxTest is Test, MainnetActors, YnClisBnbStrategyTest {
    ...
    function test_ynBNBx_deposit_to_clisBnbStrategy_SyncDeposit_Enabled(uint256 depositAmount) public {
        ...
        assertEq(totalAssetsAfter, totalAssetsBefore + depositAmount, "total assets of ynBNBx should be equal to total assets of ynBNBx before plus deposit amount");
       ...
       // Generate processor tx data to execute all transactions
        ...
        assertApproxEqAbs(ynBNBx.totalAssets(), totalAssetsAfter, 1e6, "total assets of ynBNBx should be nearly equal to total assets of before processor");
        ...
        values[0] = 0;
        data[0] = abi.encodeWithSelector(bytes4(keccak256("approve(address,uint256)")), address(clisBnbStrategy), slisBnbReceived);

        targets[1] = address(clisBnbStrategy);
        values[1] = 0;
        data[1] = abi.encodeWithSelector(bytes4(keccak256("deposit(uint256,address)")), slisBnbReceived, address(ynBNBx));
        ...
        assertEq(slisBnb.balanceOf(address(ynBNBx)), slisBnbBalanceBefore, "slisBnb balance of ynBNBx should be equal to slisBnb balance before");
        assertEq(clisBnbStrategy.balanceOf(address(ynBNBx)), expectedClisBnbShare, "clisBnbStrategy balance of ynBNBx should be equal to expected clisBnb share");
        assertApproxEqRel(clisBnb.balanceOf(MC.YIELDNEST_MPC_WALLET) - clisBnbBalanceBefore,slisBnbReceived, 0.05e18, "clisBnb balance of yieldnest mpc wallet should be approximately equal to 95% of slisBnb received");
        assertEq(clisBnbStrategy.totalAssets(), slisBnbReceived, "total assets of clisBnbStrategy should be equal to slisBnb received");
        assertEq(clisBnbStrategy.totalSupply(), expectedClisBnbShare, "total supply of clisBnbStrategy should be equal to expected clisBnb share");
        assertApproxEqAbs(ynBNBx.totalAssets(), totalAssetsAfter, 5, "total assets of ynBNBx should be equal to total assets of before processor");
    }
    ...
    function test_ynBNBx_deposit_to_clisBnbStrategy_SyncDeposit_Disabled(uint256 depositAmount) public {
       ...
       // Generate processor tx data to execute all transactions
        ...
        assertEq(slisBnb.balanceOf(address(ynBNBx)), slisBnbBalanceBefore, "slisBnb balance of ynBNBx should be equal to slisBnb balance before");
        assertEq(clisBnbStrategy.balanceOf(address(ynBNBx)), expectedClisBnbShare, "clisBnbStrategy balance of ynBNBx should be equal to expected clisBnb share");
        assertEq(slisBnb.balanceOf(address(clisBnbStrategy)), slisBnbBalanceBeforeOfClisBnbStrategy + slisBnbReceived, "clisBnb balance of slisBnb should be equal to slisBnb balance before plus slisBnb received");
        assertEq(clisBnb.balanceOf(MC.YIELDNEST_MPC_WALLET),clisBnbBalanceBefore, "clisBnb balance of yieldnest mpc wallet should be equal to clisBnb balance before");
        assertEq(clisBnbStrategy.totalAssets(), slisBnbReceived, "total assets of clisBnbStrategy should be equal to slisBnb received");
        assertEq(clisBnbStrategy.totalSupply(), expectedClisBnbShare, "total supply of clisBnbStrategy should be equal to expected clisBnb share");
        assertApproxEqAbs(ynBNBx.totalAssets(), totalAssetsAfter, 5, "total assets of ynBNBx should be equal to total assets of before processor");
    }
    ...
    function test_ynBNBx_withdraw_from_clisBnbStrategy(uint256 depositAmount) public {
        ...
        values[0] = 0;
        data[0] = abi.encodeWithSelector(bytes4(keccak256("withdraw(uint256,address,address)")), withdrawAmount, address(ynBNBx), address(ynBNBx));
        ...
        assertEq(slisBnb.balanceOf(address(ynBNBx)), slisBnbBalanceBeforeOfYnBNBx + withdrawAmount, "slisBnb balance of ynBNBx should be equal to slisBnb balance before plus withdraw amount");
        assertLe(clisBnb.balanceOf(MC.YIELDNEST_MPC_WALLET), clisBnbBalanceBefore, "clisBnb balance of yieldnest mpc wallet should be less than or equal to clisBnb balance before");
        assertEq(clisBnbStrategy.balanceOf(address(ynBNBx)) + expectedClisBnbShare, clisBnbStrategyBalanceBeforeOfYnBNBx, "clisBnbStrategy balance of ynBNBx should be equal to total supply before of ynBNBx minus expected clisBnb share");
        assertEq(slisBnb.balanceOf(address(ynBNBx)), slisBnbBalanceBeforeOfYnBNBx + withdrawAmount, "slisBnb balance of ynBNBx should be equal to slisBnb balance before plus withdraw amount");
        assertEq(clisBnbStrategy.totalAssets(), totalAssetsBeforeOfClisBnbStrategy - withdrawAmount, "total assets of clisBnbStrategy should be equal to total assets of before minus withdraw amount");
        assertEq(ynBNBx.totalAssets(), totalAssetsBeforeOfYnBNBx, "total assets of ynBNBx should be equal to total assets of before");
        assertEq(ynBNBx.totalSupply(), totalSupplyBeforeOfYnBNBx, "total supply of ynBNBx should be equal to total supply of before");
        assertEq(clisBnbStrategy.totalSupply(), totalSupplyBeforeOfClisBnbStrategy - expectedClisBnbShare, "total supply of clisBnbStrategy should be equal to total supply of before minus expected clisBnb share");
    }
    ...
    function test_reward_stream_to_clisBnbStrategy(uint256 depositAmount, uint256 rewardAmount) public {
        ...
        uint256 totalAssetsBeforeOfClisBnbStrategy = clisBnbStrategy.totalAssets();
        uint256 totalSupplyBeforeOfClisBnbStrategy = clisBnbStrategy.totalSupply();
        uint256 slisBnbLockedInClisBnbStrategyVaultBefore = _getStakedSlisBnbBalanceByVault(address(slisBnb), address(clisBnbStrategy));
        ...
        assertEq(totalAssetsAfter, totalAssetsBeforeOfClisBnbStrategy + rewardAmount, "Total assets of vault should increase by reward amount");
        assertEq(totalSupplyAfter, totalSupplyBeforeOfClisBnbStrategy, "Total supply of vault should not change");
        assertEq(slisBnbLockedInVaultAfter, slisBnbLockedInClisBnbStrategyVaultBefore, "SlisBnb locked in vault should not change");
        ...
        assertGt(ynBNBx.totalAssets(), totalAssetsBeforeOfYnBNBx, "total assets of ynBNBx should be greater than total assets of before");
        assertEq(ynBNBx.totalSupply(), totalSupplyBeforeOfYnBNBx, "total supply of ynBNBx should be equal to total supply of before");
        assertGt(ynBNBx.previewRedeem(1 ether), ynBNBxRateBefore, "ynBNBx rate should be greater than ynBNBx rate before");
    }
    ...
    function test_reward_stream_allocation_to_clisBnbStrategy(uint256 depositAmount, uint256 rewardAmount) public {
        ...
        assertEq(totalAssetsAfterOfClisBnbStrategy, totalAssetsBeforeOfClisBnbStrategy, "Total assets of vault should not change");
        assertEq(totalSupplyAfterOfClisBnbStrategy, totalSupplyBeforeOfClisBnbStrategy, "Total supply of vault should not change");
        assertEq(slisBnbLockedInVaultAfter, slisBnbLockedInClisBnbStrategyVaultBefore + rewardAmount, "SlisBnb locked in vault should increase by reward amount");
        assertGt(clisBnb.balanceOf(MC.YIELDNEST_MPC_WALLET), clisBnbBalanceOfYieldnestMpcWalletBefore, "ClisBnb balance of YieldnestMpcWallet should increase due to reward stream allocation");
        assertEq(ynBNBx.totalAssets(), totalAssetsBeforeOfYnBNBx, "total assets of ynBNBx should be greater than total assets of before");
        assertEq(ynBNBx.totalSupply(), totalSupplyBeforeOfYnBNBx, "total supply of ynBNBx should be equal to total supply of before");
        assertEq(ynBNBx.previewRedeem(1 ether), ynBNBxRateBefore, "ynBNBx rate should be greater than ynBNBx rate before");
    }
    ...
    function test_ynBNBx_withdraw_from_ynasBNBK() public {
        ...
        data[0] = abi.encodeWithSelector(
            BaseStrategy.redeemAsset.selector,
            MC.ASBNB,
            shares,
            address(ynBNBx),
            address(ynBNBx)
...

        {
            ...
            assertApproxEqAbs(
                totalAssetsOfasBNBkAfter,
                0,
                2,
                "asBNBk should have nearly 0 total assets after full redemption"
            );
            assertApproxEqAbs(
            ...
            assertApproxEqAbs(
                ...
                ynBNBxRateBefore,
                5,
                "ynBNBx rate should be equal to ynBNBx rate before"
            );
        }
    }
}
